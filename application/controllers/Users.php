...
class Users extends MY_Controller {
    ...
    function appointments($app_id="",$action=""){
        ...
        if($q->num_rows() > 0){
                        ...
                        {
                                    $user = $q->row();
                                   $this->db->update("users",array("user_password"=>_encrypt_val($this->input->post("n_password")),"varified_token"=>""),array("user_id"=>$user->user_id));
                                   $data["success"] = true;                             
                    ...
                    if(_decrypt_val($user_data->user_password) == $this->input->post("c_password")){
                        ...
                        if($n_password == $r_password){
                            ...
                            $resid = $this->common_model->data_update("users",
                                array("user_password"=>_encrypt_val($n_password)),array("user_id"=>_get_current_user_id($this)));
